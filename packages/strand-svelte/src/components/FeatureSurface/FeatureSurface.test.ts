...
import FeatureSurface from './FeatureSurface.svelte'
...
describe('FeatureSurface', () => {
  ...
  it('renders as a semantic element when asked', () => {
    ...
    const { container } = render(FeatureSurface, { props: { as: 'article' } })
  ...
  it('carries a consumer class without dropping its own', () => {
    const { container } = render(FeatureSurface, { props: { className: 'x', class: 'x' } })
