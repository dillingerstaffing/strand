...
/**
 * Determinate or indeterminate progress, as a bar or a ring.
 *
 * @example
...
export const Progress = forwardRef<HTMLDivElement, ProgressProps>(({ variant = "bar", value, size = "md", className = "", ...rest }, ref) => {
  ...
  if (variant === "ring") {
    ...
    const offset = isDeterminate ? circumference - (circumference * (value as number)) / 100 : 0;
    return (
      <div ref={ref} className={classes} {...aria} {...rest}>
        <svg width={dim} height={dim} viewBox={`0 0 ${dim} ${dim}`} className="strand-progress__ring">
          <circle cx={dim / 2} cy={dim / 2} r={radius} fill="none" stroke-width={RING_STROKE} className="strand-progress__track" />
          <circle
            cx={dim / 2}
            cy={dim / 2}
            r={radius}
  ...
  return (
    <div ref={ref} className={classes} {...aria} {...rest}>
      <div className="strand-progress__fill" style={isDeterminate ? { width: `${value}%` } : undefined} />
    </div>
  );
});
