...
describe("Progress", () => {
  ...
  it("applies bar variant class by default", () => {
    const { getByRole } = render(<Progress value={50} />);
    expect(getByRole("progressbar").className).toContain(
      "strand-progress--bar",
    );
  });

  it("applies ring variant class", () => {
    const { getByRole } = render(<Progress variant="ring" value={50} />);
    expect(getByRole("progressbar").className).toContain(
  ...
  it("renders SVG for ring variant", () => {
    const { container } = render(<Progress variant="ring" value={50} />);
  ...
  it("applies sm size class", () => {
    const { getByRole } = render(<Progress value={50} size="sm" />);
    expect(getByRole("progressbar").className).toContain(
      "strand-progress--sm",
    );
  });
  ...
  it("applies lg size class", () => {
    const { getByRole } = render(<Progress value={50} size="lg" />);
    expect(getByRole("progressbar").className).toContain(
      "strand-progress--lg",
    );
  });
  ...
  it("merges custom className", () => {
    const { getByRole } = render(<Progress value={50} className="custom" />);
