...
7. **Controlled inputs and state hoisting.** `Checkbox`, `Switch`, `Radio`, `Select`, `Input`, `Textarea`, `Slider` and `StarRating` take `value`/`checked` and `onChange`; the component holds no copy of the value. `Tabs`, `ChipSet` and `Nav` hoist their selection to the consumer through the same shape. Where the prop is left unset the component is uncontrolled: `Checkbox` and `Radio` let the native input own the state (`defaultChecked`) and paint it from `:checked`; `Switch`, `Tooltip`, `Table` and `Toast`, whose state has no native home, own it in `useState` and read `prop ?? own`. See `docs/cf/native-state-selectors.md`.
