...
### Gap #100
- Type: **L2** (library). Found by a peer consumer session measuring v0.47.0 within minutes of its release, before any consumer had synced it. Logged rather than quietly amended, because the interesting part is that the defect was created BY the fix for Gap #98 and shipped inside it.
- Symptom, measured at coarse pointer with the shipped class string: the account avatar rendered **58x34** where the design draws a **34x34** circle. A pill, on every signed-in desktop session.
- Root cause: #98's rule was written for a control with a WORD in it. `padding-inline: var(--strand-space-3)` is correct either side of "Sign in" and wrong either side of a 32px circle, and the rule applied to every `.strand-btn` in the nav's action and slot regions without distinguishing the two.
- **The premise was right and that is what hid it.** #98 states that "the avatar gets the same treatment from the same rule", and it does. The error was in believing one treatment fits both, when the whole difference between a labelled button and an icon-only one is that a shape supplies its own dimensions and a word does not.
- Fix: a second rule in the same fine-pointer block zeroing the padding and pinning 34x34 for `--icon-only`. Padding is **0 rather than 1px** because `.strand-btn` carries a 1px transparent border and the box is border-box, so a 32px avatar plus two borders is exactly 34.
- **The selector doubles `.strand-btn` deliberately.** Button's own `.strand-btn--icon-only.strand-btn--sm` is (0,2,0), so a nav rule that ties at (0,2,0) has its outcome decided by source order between two stylesheets. Doubling makes it (0,3,0), which makes the result a property of the selector rather than of the bundler. The consumer proved this the expensive way first: a single-class override at (0,1,0) lost silently and measured 44x42.
- Coarse needs no rule at all, once Gap #99's missing size modifier is present: Strand's own icon-only rule already gives 44x44. That is precisely why the missing modifier mattered, and it is the cleanest possible demonstration that #99 was a real defect rather than a tidiness note.
- Post-fix, both auth states match in height under both pointers: fine 69x34 and 34x34, coarse 69x44 and 44x44. Height parity across the swap is what #98 exists to protect, so auth resolving still moves nothing.

### Gap #101
- Type: **L2** (library), plus an **L2 tooling** finding that is the more useful half. Found by a consumer's end-to-end test failing where the library's own unit test passes.
- Symptom: `Dialog` ignores Escape for the first animation frame after it opens. Pressing Escape in that window does nothing at all.
- Root cause: Escape is bound with `onKeyDown` **on the backdrop element**, so the key only reaches the handler once focus is inside the dialog. Focus is moved in a `requestAnimationFrame` after open, and until it lands the focused element is still whatever opened the dialog. That opener is not a descendant of the backdrop, so the keydown bubbles to the document and no handler sees it.
- Impact on its own is small: a human cannot press a key inside one frame. It is logged anyway because of what it says about the test that was supposed to cover it.
- **The unit test asserting this behaviour cannot fail for the reason a user hits.** It reads:
  `fireEvent.keyDown(getByRole("dialog").parentElement!, { key: "Escape" })`
  which dispatches the event **directly on the element carrying the handler**. That asserts the handler is correct, and the handler IS correct. It cannot observe whether the event ever REACHES that element, which is the only thing that was ever in doubt. A test that hands the event to the listener is testing the listener, not the binding.
- This is the same shape as Gap #90: a property that only exists in the composition cannot be found by examining the parts. There the pairing was created by a consumer nesting two correct components; here the delivery path is created by focus position, which no jsdom dispatch models.
- The durable fix is to bind Escape at the document level while open, so dismissal does not depend on where focus happens to be. That also removes the dependence on a `requestAnimationFrame` landing first, which is a race the component currently wins only because humans are slow.
- Deliberately NOT fixed in this release. It was found while verifying a nav propagation that is measured, green and waiting on production, and a Dialog change touches every consumer of every dialog in the library. It rides with the next Nav release rather than being smuggled into a verified one.

## Production consumer: the content section under that nav
Date: 2026-08-14
Verdict: FAIL (one L2 gap, one L3 gap; both closed). Two further findings were correctly NOT gaps, and one of them is the more instructive record.

Same consumer, next surface down: the Discover listing on a real product. A dark two-pane feature card leading a list of event rows, prototyped as page-local overrides of design-system classes so the founder could approve the rendering before any of it was propagated, with every override carrying the measurement that produced it. The approval is a measured file rather than a description (22 probes at 7 viewports, plus CLS, axe and horizontal overflow), so every entry below is written under the constraint that the rendering must not move by a single cell.

**The useful ratio this time is the inverse of the nav's.** There, six of seven findings were fixed by REMOVING a rule the component should not have asserted. Here both real gaps are the opposite shape: the component asserts something correct and offers the consumer no way to say otherwise. A padding, and a font size.

**And the two non-gaps matter more than usual**, because both are consumer type sizes that are off this language's scale entirely (38px where the ladder says 39.056, and a 14px value where the scale offers 13.328 or 16). Propagating those upstream would have corrupted a Major Third scale to match one mockup's rounding. Where the consumer could compose its own size WITHOUT restyling a design-system class, it did, and that is not a gap. Where it could not, that inability was the gap. The distinction is the whole content of this iteration.

### Gap #102
- Type: **L2** (library). Tree: could a world-class designer build this with the current public API? No: `.strand-feature-surface` hardcodes `padding: var(--strand-space-6)` and there is no modifier, so the only exit is a page-local rule overriding a design-system class. Does the fix need `docs/design-language.md` changed? No: 9.3 specifies the surface's ROLE, background and text cascade and says nothing about its inset, and Card already ships the tier ladder being added here. Therefore L2.
- Symptom: the consumer's lead card is a two-pane grid whose divider has to run the **full height of the card**. That requires each PANE to carry the 32px inset and the surface to carry none. The surface pads itself, so the consumer shipped `.stg-featured { padding: 0 }` -- a page-local override of a design-system class, which is precisely what the strand-first gate exists to forbid.
- **The equal-specificity trap underneath it, which is the part worth carrying.** `.stg-featured` and `.strand-feature-surface` are both (0,1,0). The override therefore won by SOURCE ORDER, not by specificity, and it won only because the consumer's sheet arrives in a dynamically-imported chunk appended after the static link. A card's geometry decided by bundler output ordering is correct by accident. `padding="none"` makes it a property of the API.
- Fix: `--pad-none` through `--pad-xl`, the same five tiers and the same values Card ships, plus a `padding` prop on all three framework components defaulting to `md`. The base keeps `space-6` so a CSS-only consumer writing the bare class is unaffected, and `--pad-md` restates it so the emitted class can never disagree with the rendered box.
- **A capability present on Card and absent on FeatureSurface is a hole, not a distinction.** These are the two surface primitives a consumer builds a card out of. Nothing about being midnight rather than elevated argues for owning your inset, and the asymmetry existed for no reason anyone could state.
- **The clip travels with `--pad-none` rather than sitting on the base, and the placement is a decision rather than a detail.** A pane laid against a radius-lg corner squares it off unless the surface clips, so `--pad-none` without `overflow: hidden` is a modifier that does not work out of the box (Principle 8). Putting it on the base would instead change overflow behaviour for every existing consumer that never asked for it; Card can clip unconditionally only because Card always has.

### Gap #103
- Type: **L3** (design language). Tree: no rung on the ladder produces this, AND the rung cannot be added without changing the spec, because 11.2 said in as many words that "the overline label stays at `--strand-text-xs` across all sizes". A rung that moved the label would have made the library contradict the language; a rung that did not would have shipped a label louder than its value. Therefore L3, and the spec moved first.
- Symptom, measured on the rendered page rather than read off the source: the card's two meta facts (`Channel / Weekly Ship`, `Meet at / Remote`) need a 10px label over a 14px value. Strand's default value is **39.056px** and the smallest rung it ships, `--sm`, is **25.008px**. So the consumer restyled `.strand-data-readout__label` and `.strand-data-readout__value` directly.
- **At the default the readout rendered LARGER than the card's own 32px headline.** That is a hierarchy inversion rather than an overshoot: the venue name was the loudest thing on a card built around an event title.
- **The root cause is structural and is best seen by comparing with Headline.** `.strand-headline` ships family, weight, case, colour and leading on the BASE and puts every size in a modifier, so a product needing an off-ladder display size composes the base and supplies its own size, overriding nothing. `.strand-data-readout__label` and `__value` declare `font-size` on the base itself. There is no size-less base to compose, so there is no way off the ladder that is not a BEM-internal override.
- **Why the readout has two ends and the ladder only reaches one.** Every rung scales the VALUE and holds the label, because the ladder describes an INSTRUMENT: a number that is the hero with a caption naming it. The same stacked pair is also the grammar's `inline-pair` (11.10) used as a LABELLED FACT, where the value is not a number, the label is not a caption, and both halves are set at caption scale because both carry meaning. 11.13's own inline-pair test says so: cover either half and the row must fail.
- Fix, in order: **11.2.1 added** to state that the sizes are a per-instance API and to scope the label rule to the modifiers, then `--strand-data-readout-label-size` and `--strand-data-readout-value-size` in the stylesheet, then the size modifiers rewritten to set the value property instead of a `font-size`.
- **The fallback lives INSIDE the `var()`, and that is load-bearing rather than stylistic.** `.strand-data-readout__value` appears in real consumer markup with no `.strand-data-readout` ancestor, composed with `.strand-instrument-viewport__value`. Declaring the custom property on the block would leave every such orphan resolving an unset variable, which is invalid at computed-value time -- and `font-size` is an INHERITED property, so the failure mode is not "no size" but "silently takes its parent's size". A test asserts the fallback stays inside the var for exactly this reason.
- **Modifiers set the property on the BLOCK, so the ladder stays authoritative.** Custom-property resolution takes the nearest declaring ancestor and the modifier sits on the readout itself, so `--sm` beats a consumer's declaration further up the tree. A consumer wanting to override a modifier must say so on that readout.
- **Size only.** Family, weight, tracking, tabular numerals and the on-dark cascades are not settable and are not negotiable: they are what makes a readout a readout (Principle 10). A consumer needing to change those does not want this component.
- **Recorded because the handoff that produced it carried a wrong number in the grammar of a measured one.** It said the default value was "the body scale (16)". It is 39.056. The finding survived being wrong -- the gap is real and worse in degree than the wrong number suggested -- but the receiving session only learned that by mounting a bare readout outside the override and measuring it. A first re-check using `font-size: revert` on the live element reported 16px and was invalid: `revert` drops to the INHERITED value, not to the library's own rule, so it measures the UA default and looks like a clean answer.

### Not a gap: the consumer's 38px headline
- The page's `<h1>` renders at 38px/300/-0.03em. `.strand-headline--md` is a fluid clamp topping out at 28px and `--lg` reaches 40px at this width, so no rung matches, and the prototype had reached into `.strand-headline` to set the size.
- **It stayed in the consumer, and the composition is the sanctioned one rather than a deferral.** `.strand-headline` deliberately ships without a size: every size is a modifier. So the base plus a product-owned size class IS the API, and the product's class carries `font-size` and `letter-spacing` together for the same reason every Strand size modifier does (4.3: large text tighter, small text looser). Dropping `--md` from the element is what turns an override into a composition.
- 38px is not on this language's scale and should not be. The scale is a Major Third and its neighbour is 39.056px; the difference is one mockup's rounding, and a design system that absorbed it would be taking dictation from a PNG.

### Not a gap: the event row's 20px/24px inset
...
- **The first fix was transparent, and the check is what said so.** It fell back to `--strand-surface-raised`, which does not exist. A `var()` whose fallback is itself an undefined `var()` resolves to nothing, and `background: <nothing>` is TRANSPARENT: the bar measured `rgba(0,0,0,0)`, strictly worse than the glass it replaced. It survived review because the name reads exactly like a real token, and it was caught by measuring the rendered bar rather than by reading the rule. A test now asserts the fallback names a token the token file actually defines, because this failure mode is invisible in the CSS and total in the output.
...
## Production consumer: the calendar screen and the shell it sits in

Same consumer, next surface again: a month grid with a channel rail beside it,
plus the placeholder pages that share its shell. Both gaps below were found the
same way and are the same shape underneath -- a rule that was applied to a
WIDER set of things than the fact justifying it covers. One clipped every grid
because containers clip; one reserved thumb-bar space at every width because a
phone needs it.

### Gap #114
- Type: **L2** (library). Tree: no spec change, because the spec already draws the distinction this rule collapsed.
- Symptom, measured on the consumer's calendar header at 1440: the three month controls (Today, previous, next) sat at **slackTop 0** against `.strand-grid`'s content edge, and the next-month button also at **slackRight 0**. `Button` hover is `transform: translateY(-1px)` plus `box-shadow: var(--strand-elevation-1)`, and `.strand-grid` computed `overflow: clip`, so the lift and the entire shadow were cut off on every hover. The founder reported it as the arrow borders "clipping a little".
- **The rule was not wrong so much as over-applied, and 10.4 already says so.** It names two remedies for two different kinds of thing: *"Container components apply `overflow: hidden` and ensure children cannot exceed the content box. Layout primitives apply `min-width: 0` to their children so flex/grid items shrink correctly rather than overflowing."* A grid is the second kind. It has no padding, so it has no padding zone to hold inviolable, and clipping it protected nothing while costing the hover affordance Part XI mandates.
- This is the call **gap #102** already made for Card and stated as a rule: *"Putting it on the base would instead change overflow behaviour for every existing consumer that never asked for it; Card can clip unconditionally only because Card always has [padding]."* A grid never has.
- Fix: `.strand-grid` declares no `overflow` at all.
- **The clip was also hiding something, so removing it alone would have shipped a regression.** A bare `1fr` track floors at min-content, so one long unbroken string could widen a fixed-column grid past its container and the clip painted over the result. Every fixed track is now `minmax(0, 1fr)` -- in the `--cols-*` utilities, in the Preact inline style, and in the Svelte and Vue equivalents -- which is the fix **gap #372** landed for LabShell for exactly this reason. The protection is structural instead of painted over, and a new layout case measures the GRID (not the child) staying inside a 600px container with an unbreakable string in it.
- **The guard has to read the stylesheet, and that is a property of the defect rather than a preference.** Clipping is a PAINT operation: it changes no box, so `getBoundingClientRect` is byte-identical clipped or not. jsdom cannot see it and neither can the real-Chromium layout tier. Same instrument, same reason, as #113's clipped-not-hidden label guard.
- **The first version of that guard failed against its own comment**, which contains the word "overflow" in prose. A source guard that matches commentary is measuring what a rule SAYS rather than what it DOES, and it would have passed and failed for the wrong reasons in both directions. Comments are stripped first, and a guard-on-the-guard asserts the stripping still works.
- The sticky-rail layout case is KEPT rather than retired. It has now outlived three different causes (`hidden` making a scrollport, then `clip`, now no overflow at all) and it is the assertion that catches anyone re-adding an overflow here for a fourth reason.

### Gap #115
- Type: **L2** (library). Tree: no spec change; 19.1.1 already states the condition the utility did not implement.
- Symptom, measured at 1440: `.strand-tabbar-offset` applied **76px of `padding-block-end` beneath a bar computing `display: none`** -- on the one screen whose entire design is that a month is legible without scrolling. The consumer had stood it in for locally, which is what surfaced it.
- **19.1.1 closes with the answer**: *"Desktop is unchanged. Reach is a property of a touch viewport; a horizontal bar at the top of a pointer-driven layout carries none of the costs above."* The bar is a touch-viewport object and its reservation was not, so the two disagreed about where the bar exists.
- Fix: the reservation moves inside `@media (max-width: 767.98px)`, the exact complement of `strand-hide-from-md`, which is the composition this bar documents for hiding itself. The two thresholds are now restated **together, in one place**, so they cannot drift into a gap (no reservation under a visible bar) or an overlap (padding under no bar).
- **The single-token contract is unchanged and deliberately so.** The reservation still reads `var(--strand-tabbar-height)`, so a taller bar still cannot outgrow its space, and a consumer hiding the bar at a DIFFERENT width owns both halves of that decision by setting the token to `0px` at its own breakpoint rather than by overriding the utility.
- Three guards, and the middle one is the one that matters: the declaration must sit INSIDE the media query (an unconditional rule plus a media query restating it would leave desktop exactly as it was while looking fixed), the token and the safe-area inset both survive (the inset is the half a headless browser resolves to 0, so it is asserted from source rather than measured), and the bundle contains **exactly one** `.strand-tabbar-offset` rule, so a second unconditional copy anywhere cannot quietly re-add the 76px.

### Gap #116
- Type: **L2** (library). Tree: no spec change; 10.6 already permits a taller floor, it just gave no way to ask for one.
- Symptom: the consumer's calendar carried `.stg-cal-main .strand-calendar-grid__day { min-block-size: clamp(72px, 11vh, 132px) }` -- a consumer rule reaching past the API into a design-system class, which is the workaround the strand-first gate exists to refuse.
- **Found by classifying a prototype stylesheet for deletion rather than by a complaint**, which is the process working: every block gets read as UPSTREAM or CONSUMER-PERMANENT before the file goes, and this one was neither. It was a stand-in for a primitive that did not exist.
- **It is load-bearing for an approved rendering, which is what made it urgent.** The founder-approved cell measures 99px at a 900px viewport; the default floor is `--strand-space-20`, 80px. Deleting the prototype without an upstream home would have silently moved an approved measurement by 19px per row, six rows deep, and the baseline would have caught it as unexplained drift.
...
### Gap #121
- Type: **L2** (library). Tree: no spec change; the chip is an existing primitive whose parts were separable, and nothing in `docs/design-language.md` needed a new token, shape or pattern to say they should not be.
- Symptom, as the consumer reported it: *"the symbol next to Online causes text to wrap low and throws off the perceived alignment of all the tags in the row... this looks wrong any location this online tag is used."*
- **Measured cause.** `.strand-status-chip` was `display: inline-block` with no `white-space` and no flex, so the glyph and the label were two INDEPENDENT inline children and the only line-break opportunity in the entire chip sat between them. A width sweep puts the break at 66px: at 70px the chip is 25px tall and at 66px it is 41px, with the glyph orphaned onto its own line 17.6px above its label. The label is one unbreakable word throughout, so the break can land nowhere else.
- **The chip was undoing its own co-class.** `.strand-badge` already declares `inline-flex`, and consumers apply both together (`class="strand-badge strand-status-chip strand-status-chip--neutral"`). At equal specificity the later rule won, so the chip silently reverted the badge's flex display and supplied neither a `gap` nor any wrap protection in its place. The measured icon-to-label gap was therefore **0 at every width**, not only at narrow ones: this was a defect on every icon chip ever rendered, and the wrap was only its most visible form.
- **Every icon-bearing sibling in the library already gets this intrinsically**: `.strand-breadcrumb__item` (`inline-flex`, `align-items: center`, `gap: space-2`), `.strand-star-rating` (`gap: space-1`). The chip was the inconsistent one, which is what settles this at L2 rather than L1: the composition a consumer would have to reach for is a pair of generic utilities whose only job is to counteract the component.
- Fix: `display: inline-flex; align-items: center; gap: var(--strand-space-1)` plus `.strand-status-chip > svg { flex: none }`.
...
- **What is NOT claimed**: that the design's numbers are automatically right. It may have been drawn in a tool with no scale. The test is whether the RENDERING is better, and that test has now been run: the consumer prototyped at the design's numbers, the founder reviewed it against the snapped version, and the question was raised by the founder rather than inferred — *"is the scale really raising the bar at display sizes, or is 52 actually an improvement to Strand?"*
...
- **Fixed in 0.56.0**: `variant="instrument"` on `Breadcrumb`, matching the pattern `StatStrip` already has with `variant="bordered"`. Mono, uppercase and tracked to sit beside the overline and status-chip labels a tinted header puts next to it. The link keeps `blue-deep` rather than `blue-primary`, because at that size 1.4.3 governs and the two measure 5.99:1 against 3.29:1 on the light ground these headers use.
- **The variant is PAINT, never structure**, and a test asserts exactly that: the rendered trail with the modifier stripped is byte-identical to the default. A variant that quietly reorders or relabels a trail would be a second breadcrumb wearing one name.
- The consumer that surfaced this had composed its own trail from product-owned classes, which was right at the time (overriding a primitive's internals is worse) and is now unnecessary. It consumes the variant and deletes its copy.
