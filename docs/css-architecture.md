...
4. **State is an attribute where the platform has one.** `[aria-current="page"]`, `[aria-pressed="true"]`, `[data-strand-reserve="ready"]`, `:checked`, `:disabled`. A class exists for a state only when no attribute carries it, so the painted state and the announced state cannot drift.
